...
This page explains how `nuci` works under the hood — the mechanisms that make
deploys **idempotent**, **safe**, and **reload-aware** on a real OpenWrt device.
...
so every output — UCI batch, package installs, file writes, raw UCI escape lines —
is derived from a single `Root` model. That is what keeps the system idempotent:
there is exactly one function that decides "what the router should look like".
...
### Named sections → `delete` + `set`

A named section (e.g. `network.lan`) is addressed by a stable name. To avoid
...
The `delete` first wipes the section, then `set` rebuilds it from the Nix model.
Result: removing an option from Nix **removes it from the router** — no drift.

### Anonymous list sections → `while uci -q delete @type[0]`

Anonymous sections (e.g. `wireless.@wifi-iface[0]`) have **no stable name**, so
you cannot address "the third one" reliably across redeploys. `nuci` clears them
by repeatedly deleting the head until the list is empty, then re-adds from the model:
...
This is a deliberate full-rebuild strategy: anonymous sections do not have a
stable identity to match against, so the only correct idempotent move is
"clear and rebuild". (This is also why the audit's "stable identity for anon
...
(`\u{1f}`, defined as `LIST_SEP` in `diff.rs`) before comparing, so a reordered
list is correctly recognised as **no change**. The separator is stripped back to
a human-readable `a, b, c` form only for display in `nuci diff`.
...
## Core mechanism 2 — The anti-brick safety net

A bad network deploy can lock you out of a router 500 km away. `nuci` ships a
**two-layer** rollback net.

### Layer A — Watchdog (60s, in-session)

Before applying changes, `nuci` snapshots `/etc/config` to
`/etc/.uci-rollback-backup` and spawns a background watchdog:
...
IP, dead interface, reboot), the watchdog fires after 60 s, restores the backup,
and reloads. The timeout is overridable via `NUCI_WATCHDOG_TIMEOUT`.

### Layer B — Boot-time hook (power loss during deploy)

The watchdog only helps if the box stays up. If it **reboots mid-deploy**
(power loss), `nuci` installs a procd-style init script `S15nuci_rollback`
that, on next boot, restores `/etc/config` from the backup and then
**deletes itself**:
...
Together, the two layers cover: transient network failure (A) **and** hard
power loss (B).

## Core mechanism 3 — Smart service reload
...
2. **Fallback:** when procd isn't PID 1 (e.g. a container, or minimal setups),
   `nuci` runs a discovery command that greps `/etc/init.d/*` for the official
   `config_load <config>` directive to learn **which services own which config**,
   then reloads only the affected ones (`/etc/init.d/<svc> reload`). This
   heuristic is documented in `diff.rs` as the upstream OpenWrt convention —
   no behaviour change, just an officially-cited fallback.

`uci diff` prints the discovered affected services, making the reload preview
explicit:
...
## Core mechanism 4 — Orphan deletion
...
full-rebuild strategy in mechanism 1. Named-section orphans are the only ones
with a stable key to target, and `nuci` targets exactly them — so the router
converges to the Nix model with no leftovers.
